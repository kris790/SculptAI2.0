...
const MainApp = () => {
  ...
  // For dummy data demonstration, we allow a "Guest" state if not logged in
  const isGuest = !user;
  const activeUser = user || { email: 'guest@sculptai.demo', id: 'dummy-id' };
  ...
  const generatePhysiqueInsight = async () => {
    ...
    try {
      ...
      setAiInsight("Maintain consistent progressive overload in your lat pull-downs to widen that V-taper frame. Your dedication today defines your form tomorrow.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 selection:bg-indigo-500/30">
      ...
      {/* Mobile-Friendly Floating Action Button for Logging */}
      {activeTab !== 'log' && activeTab !== 'history' && (
         <button 
          onClick={() => setActiveTab('log')}
          className="fixed bottom-6 right-6 w-14 h-14 bg-indigo-600 rounded-full shadow-2xl shadow-indigo-500/40 flex items-center justify-center text-white text-2xl hover:scale-110 active:scale-95 transition-transform z-40 md:hidden"
